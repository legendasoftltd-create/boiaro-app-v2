...
//     try {
//       final res = await EbookGroup.getBookChaptersApiCall.call(
//         bookId: _bookId,
//         token: FFAppState().token,
//       );
...
//             ? (decoded['chapters'] ?? decoded['tracks'])
//             : null;
//         if (list is List) {
//           _v2Tracks = list
//               .whereType<Map>()
...
//             return;
//           }
//           final chapterId = chapter['id'] ?? chapter['_id'] ?? chapter['raw']?['id'] ?? chapter['raw']?['_id'] ?? '';
//           final chapterTitle = chapter['title'] ?? 'Chapter';
//           final coinPrice = _toInt(chapter['raw']?['chapter_price_coins'] ?? chapter['chapter_price_coins']);
//           final bdtPrice = _toNum(chapter['raw']?['chapter_price_bdt'] ?? chapter['chapter_price_bdt']) ?? 0.0;
          
//           if (chapterId.toString().isEmpty) {
//             await actions.showCustomToastBottom('Unable to unlock this chapter (invalid ID)');
//             return;
//           }
...
//             context: context,
//             builder: (ctx) => AlertDialog(
//               title: Text('Unlock $chapterTitle'),
//               content: Text(
//                 'Unlock this chapter using either coins or online payment.\n\n'
//                 '• Coin cost: ${coinPrice > 0 ? "$coinPrice" : "Not available"} coins\n'
//                 '• Cash cost: ${bdtPrice > 0 ? "৳$bdtPrice" : "Not available"}'
//               ),
//               actions: [
//                 TextButton(
...
//               await actions.showCustomToastBottom('Insufficient coins. Watch ads to earn coins!');
//               return;
//             }
//             final res = await EbookGroup.unlockChapterWithCoinsCall.call(
//               trackId: chapterId.toString(),
//               token: FFAppState().token,
//             );
